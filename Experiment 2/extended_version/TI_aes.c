...
void aes_encrypt(unsigned char *input, unsigned int inputLen, unsigned char *output, unsigned char *key) {
    ...
    unsigned char paddedInput[256]; // Adjust size if needed
    unsigned int paddedInputLen;

    padData(input, inputLen, paddedInput, &paddedInputLen);
    expandKey(expandedKey, key);

    for (unsigned int i = 0; i < paddedInputLen; i += 16) {
        aes_encr(paddedInput + i, expandedKey);
    }

    memcpy(output, paddedInput, paddedInputLen); // Copy encrypted data to output
}

void aes_decrypt(unsigned char *input, unsigned int inputLen, unsigned char *output, unsigned char *key) {
    unsigned char expandedKey[176];
    unsigned char decryptedData[256]; // Adjust size if needed
    unsigned int decryptedDataLen;

    expandKey(expandedKey, key);

    for (unsigned int i = 0; i < inputLen; i += 16) {
        aes_decr(input + i, expandedKey);
    }

    unpadData(input, inputLen, decryptedData, &decryptedDataLen);
    memcpy(output, decryptedData, decryptedDataLen); // Copy decrypted data to output
}
