...
// inverse sbox [256] which is inverse one
...
// Direct aes encryption which is  implemented below
...
void aes_encr(unsigned char *state, unsigned char *expandedKey)
{
  ...
  // tenth round without mixing of columns
  state[ 0]  = sbox[(state[ 0] ^ expandedKey[(round*16)     ])];
  state[ 4]  = sbox[(state[ 4] ^ expandedKey[(round*16) +  4])];
  state[ 8]  = sbox[(state[ 8] ^ expandedKey[(round*16) +  8])];
  state[12]  = sbox[(state[12] ^ expandedKey[(round*16) + 12])];
...
//   substeps is the exact reverse of encryption
//   inverse functions:
//       - addRoundKey to its inverse
//       - rsbox which is inverse of the sbox
//       - initially it was left shift but now right shift in decryption
...
void aes_decr(unsigned char *state, unsigned char *expandedKey)
{
  ...
  // initial (adding of round key) addroundkey
  state[ 0]^=expandedKey[160];
  state[ 1]^=expandedKey[161];
  state[ 2]^=expandedKey[162];
  state[ 3]^=expandedKey[163];
  ...
  // tenth round which do not indicate mixing of columns
...
void padData(unsigned char *input, unsigned int inputLen, unsigned char *output, unsigned int *outputLen) {
    unsigned int padLen = 16 - (inputLen % 16);
    for (unsigned int i = 0; i < inputLen; ++i) {
        output[i] = input[i];
    }
    for (unsigned int i = 0; i < padLen; ++i) {
        output[inputLen + i] = padLen;
    }
    *outputLen = inputLen + padLen;
}

void aes_encrypt(unsigned char *state, unsigned int stateLen, unsigned char *key) {
    unsigned char expandedKey[176];
    unsigned char paddedState[256]; // Ensure this is large enough to hold the padding
    unsigned int paddedStateLen;

    padData(state, stateLen, paddedState, &paddedStateLen);

    expandKey(expandedKey, key);

    // Encrypt each 16-byte block
    for (unsigned int i = 0; i < paddedStateLen; i += 16) {
        aes_encr(paddedState + i, expandedKey);
    }
}

void unpadData(unsigned char *input, unsigned int inputLen, unsigned char *output, unsigned int *outputLen) {
    unsigned int padLen = input[inputLen - 1];
    *outputLen = inputLen - padLen;
...
void aes_decrypt(unsigned char *state, unsigned int stateLen, unsigned char *key) {
    unsigned char expandedKey[176];
    unsigned char unpaddedState[256]; 
    unsigned int unpaddedStateLen;

    expandKey(expandedKey, key);

    // Decrypt each 16-byte block
    for (unsigned int i = 0; i < stateLen; i += 16) {
        aes_decr(state + i, expandedKey);
    }

    unpadData(state, stateLen, unpaddedState, &unpaddedStateLen);
}
ß